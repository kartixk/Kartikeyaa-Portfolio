    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          for(float i=0.;i++<35.;){vec2 v=p+cos(i*i+(iTime+p.x*.08)*.025+i*vec2(13.,11.))*3.5;float tailNoise=fbm(v+vec2(iTime*.5,i))*.3*(1.-(i/35.));vec4 auroraColors=vec4(.05+.05*sin(i*.2+iTime*.4),.55+.3*cos(i*.3+iTime*.5),.85+.15*sin(i*.4+iTime*.3),1.);vec4 currentContribution=auroraColors*exp(sin(i*i+iTime*.8))/length(max(v,vec2(v.x*f*.015,v.y*1.5)));float thinnessFactor=smoothstep(0.,1.,i/35.)*.6;o+=currentContribution*(1.+tailNoise*.8)*thinnessFactor;}
        ...
        <nav className="w-full px-6 py-4">
          <div className="max-w-7xl mx-auto flex justify-between items-center">
            ...
            <div className="hidden md:flex items-center space-x-8">
              {navLinks.map(link => (
                <a key={link.label} href={link.href} className="text-muted-foreground hover:text-foreground transition-colors text-sm">
                  {link.label}
                </a>
              ))}
            </div>
            <button
              ...
              className="px-4 py-2 rounded-lg text-foreground text-sm font-medium border border-white/20 bg-white/5 backdrop-blur-md hover:bg-white/10 transition-all"
            >
              {resume.label}
            </button>
          </div>
        ...
        <main id="about" className="w-full min-h-screen flex flex-col items-center justify-center px-6 py-20">
          <div className="max-w-6xl mx-auto text-center">
            <div className="mb-8 animate-[float_4s_ease-in-out_infinite]">
              ...
              <p className="md:text-xl max-w-3xl leading-relaxed text-lg font-light text-muted-foreground mx-auto">
                {hero.subtitle}
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16">
              <button
                ...
                className="px-6 py-3 text-white rounded-lg font-medium text-sm min-w-[160px] bg-primary hover:bg-primary/90 transition-colors"
              >
                {ctaButtons.primary.label}
              </button>
              <button
                onClick={ctaButtons.secondary.onClick}
                className="min-w-[160px] text-sm font-medium text-foreground rounded-lg px-6 py-3 border border-white/20 bg-white/5 backdrop-blur-md hover:bg-white/10 transition-all"
              >
            ...
            <div id="projects" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto mb-16">
              {projects.map((project, index) => (
                <div key={index} className="glass-card rounded-2xl p-6 text-left hover:border-primary/40 transition-all duration-300">
                  <div className="bg-muted/30 rounded-xl h-32 mb-4 flex items-center justify-center border border-white/5">
                    {project.imageContent ?? <span className="text-muted-foreground text-sm">{project.title}</span>}
                  </div>
                  <h3 className="text-lg font-medium text-foreground mb-2">{project.title}</h3>
                  <p className="text-muted-foreground text-sm mb-4">{project.description}</p>
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map(tag => (
                      <span key={tag} className="px-2 py-1 rounded text-xs text-muted-foreground bg-muted/50 border border-border/40">
                        {tag}
                      </span>
                    ))}
                  </div>
